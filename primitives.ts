...
export const parser = (p: any) => (input: string) => p(input)

export const char = (c: string) => (input: string) =>
  input[0] === c ? [input[0], input.slice(1)] : [null, input]

export const or = (p1: any) => (p2: any) => (input: string) =>
  fails(p1(input)) ? p2(input) : p1(input)
...
export const apply = (f: any) => (p: any) => (input: string) =>
  fails(p(input)) ? [null, input] : f(p(input))

export const between =
  (p1: any) => (p2: any) => (p3: any) => (input: string) => {
    const result1 = p1(input)
    if (fails(result1)) return [null, input]
